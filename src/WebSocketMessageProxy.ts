...
import type { SockMsg } from "./WebSocket";
...
export class WebsocketMessageProxy {
	...
	constructor(server: HttpsServer | HttpServer, authenticationMiddleware: (request: IncomingMessage, next: (err: any) => void) => void = (_, next) => { next(undefined); }) {
	    this.init();
