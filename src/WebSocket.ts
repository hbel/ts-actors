...
export class Answer<T> implements SocketMessage {
	 ...
	 */
	constructor(public id: string, public originId: string, public targetId: string, public questionId: string, public payload: T) {}
}
...
export class WebsocketClient {
	...
	constructor(proxy: string, private id: string, private onMessage?: (origin: string, questionId: string, payload: any) => void) {
	...
	private handleTimeouts = () => {
	    ...
	    rejectAcks.forEach(a => {
	        const ack = this.acks.get(a);
	        if (ack) {
	            const [,,reject] = ack;
	            reject?.(new Error(`ACK for message ${a} is missing`));
	            this.acks.delete(a);
	        }
	    });
	    ...
	    rejectQuestions.forEach(a => {
	        const q = this.questions.get(a);
	        if (q) {
	            const [,,reject] = q;
	            reject?.(new Error(`Answer for message ${a} is missing`));
	            this.questions.delete(a);
	        }
	    });

	}

	private init(proxy: string) {
	    this.client = new WebSocket(proxy);
	    this.client.onopen = () => {
	    ...
	    this.client.onmessage = <T>(data: MessageEvent) => {
	        ...
	        switch(d.type) {
	            ...
	            case "ack": {
	                ...
	                const [,resolve] = this.acks.get(d.id) ?? [0, () => {}, () => {}];
	                resolve?.();
	            ...
	            case "answer": {
	                // eslint-disable-next-line @typescript-eslint/no-empty-function
	                const [,resolve] = this.questions.get(d.questionId) ?? [0, () => {}, () => {}];
	                resolve?.(d.payload);
